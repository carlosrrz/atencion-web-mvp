...
let thr  = {
  enter:{ ar:0.58, off:0.28, yaw:0.24, pitch:0.12, gaze:0.35, gH:0.28, gV:0.28, mouth:0.30 }, // mouth↑
...
function loop(){
  ...
  if (landmarker && frameCount % DETECT_EVERY === 0){
    ...
    try{
      ...
      // sin rostro → oclusión tras umbral
      if (!lm){
        occlClearSince = null;
      ...
      if (lm){
        ...
        if (area >= MIN_FACE_AREA){
          ...
          if (M && typeof M[0] === 'number'){
            // usamos solo si es muy consistente con ojos
          ...
          // labios (más estricto: se exige amplitud/velocidad o boca claramente abierta)
...
// evidencias ante alerta off-tab del logger
...
function showSummaryModal(summary){
  ...
  summaryBody.innerHTML = `
    ...
    <ul>
      <li>Episodios (≥ ${summary.tab_activity.threshold_ms/1000}s): <strong>${summary.tab_activity.off_episodes}</strong></li>
    ...
    <ul>
      ...
      <li>Más largo: <strong>${fmt(summary.attention.lookaway_longest_ms)}</strong></li>
    </ul>
  ...
  btnSumJSON?.onclick = ()=>{
    ...
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download='resumen_sesion_total.json'; a.click(); URL.revokeObjectURL(a.href);
  };
  btnSumCSV?.onclick = ()=>{
    const rows = [
