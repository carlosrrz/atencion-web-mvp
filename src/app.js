// app.js — MVP + Proctoring ligero (poses + evidencias)
import { createMetrics } from './metrics.js';
import { createTabLogger } from './tab-logger.js';
import { createPoseLogger } from './pose-logger.js';
...
const lipsEl    = document.getElementById('lips-state'); // opcional en HTML
...
// Mirada desviada (perfil/lateralidad) con histéresis
const ENTER_AR       = 0.62;  // entrar si ancho/alto < 0.62
const EXIT_AR        = 0.70;  // salir si > 0.70
const ENTER_OFFX     = 0.22;  // entrar si |offset nariz| > 0.22
const EXIT_OFFX      = 0.18;  // salir si < 0.18
const SCORE_ENTER    = 12;    // integrador (sube al “salirse”)
const SCORE_EXIT     = 6;
const MIN_FACE_AREA  = 0.05;  // ignora caras muy chicas (ruido)

// Mirada arriba/abajo y labios (blendshapes)
const GAZE_V_STRICT  = 0.40;  // umbral mirada vertical estricta
const GAZE_H_STRICT  = 0.50;  // umbral mirada horizontal estricta
const MOUTH_ENTER    = 0.38;  // posible habla
const MOUTH_EXIT     = 0.30;

// Off-tab para UI acumulada
...
function loop(){
  ...
  if (frameCount % 10 === 0){
    ...
    // Atención/Labios (se actualiza más abajo con detección)
  }

  /* --------- Detección de rostro / mirada / labios --------- */
  if (landmarker && frameCount % DETECT_EVERY === 0) {
    ...
    if (cam.currentTime !== lastVideoTime) {
      ...
      if (!lm) {
        ...
        // oclusión si área demasiado pequeña
        ...
        gazeH = (outR + inL) - (outL + inR);  // -1..1 aprox
        gazeV = (up) - (down);                // -1..1 aprox
        ...
        // Flags estrictos por mirada (opcional para UI)
        const strictUpDown = Math.abs(gazeV) > GAZE_V_STRICT;
        const strictLR     = Math.abs(gazeH) > GAZE_H_STRICT;

        // Integrador de “mirada desviada”
      ...
      // Pose logger (cada ~200 ms)
      poseLogger.log({
        yaw: yawProxy, pitch: pitchProxy, mouth, occl,
        gazeH, gazeV, lookAway: isLookAway, inTab: nowVisible
      });
    }
  }

  // Tiempo fuera acumulado para UI
...
/* ===== TabLogger → snapshot de evidencia en alerta ===== */
tabLogger.setOnAlert?.((type) => {
  if (type === 'off_tab') {
    evidence.snapshot('alert/off_tab', 'Fuera de pestaña ≥ umbral', cam);
  }
});
...
btnStop?.addEventListener('click', ()=>{
  // cerrar episodio de off-tab si sigue fuera
  if (offTabStart != null){
    const now = performance.now();
    const dur = now - offTabStart;
    if (dur >= OFFTAB_EP_MS) offTabEpisodes += 1;
  ...
  tabLogger.stopAndDownloadCSV?.();               // actividad_pestana.csv
  poseLogger.stop();
  poseLogger.downloadCSV('poses.csv');
  poseLogger.downloadSummaryJSON('poses_summary.json');
  // evidencias: descarga opcional desde el botón del modal
});
